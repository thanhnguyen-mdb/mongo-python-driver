...
#                       Default: pyproject.toml requirements.txt **/requirements*.txt
...
MANIFEST_PATTERNS="${MANIFEST_PATTERNS:-pyproject.toml requirements.txt **/requirements*.txt}"
...
for pattern in $MANIFEST_PATTERNS; do
  if [[ "$pattern" == *"**"* ]]; then
    ...
    # Remove ** and use simple pattern matching
    simple_pattern=$(echo "$pattern" | sed 's|\*\*/||g')
    matches=$(git ls-files | grep -E "${simple_pattern//\*/.*}" || true)
  else
    # Direct git ls-files for simple patterns
